...
func TestSetCoinType(t *testing.T) {
	...
	require.Equal(t, int(xte.Bip44CoinType), int(config.GetCoinType()))
...
func TestHDPath(t *testing.T) {
	params := *hd.NewFundraiserParams(0, xte.Bip44CoinType, 0)
	hdPath := params.String()
	require.Equal(t, "m/44'/665'/0'/0/0", hdPath)
	require.Equal(t, hdPath, xte.BIP44HDPath)
}
