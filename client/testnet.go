...
import (
	...
	"github.com/xtelabs/xtechain/server/config"
	...
	"github.com/xtelabs/xtechain/testutil/network"
)
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "xted", "Home directory of the node's daemon configuration")
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("xtechain_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, xte.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(xte.AttoPhoton, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &xte.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, xte.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(xte.AttoPhoton, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(xte.AttoPhoton)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, xte.AttoPhoton, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
