...
import (
	...
	"github.com/xtelabs/xtechain/app/ante"
	"github.com/xtelabs/xtechain/crypto/ethsecp256k1"
	"github.com/xtelabs/xtechain/tests"
	evmtypes "github.com/xtelabs/xtechain/x/evm/types"
	...
	testCases := []struct {
		...
		},
		{
			...
			func() sdk.Tx {
				...
				txBuilder := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				txBuilder := suite.CreateTestEIP712TxBuilderMsgDelegate(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgCreateValidator(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgCreateValidator2(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				txBuilder := suite.CreateTestEIP712SubmitProposal(from, privKey, "xtechain_9527-1", gas, gasAmount, deposit)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				suite.Require().NoError(err)
				return suite.CreateTestEIP712SingleMessageTxBuilder(privKey, "xtechain_9527-1", gas, gasAmount, msg).GetTx()
			}, false, false, true,
		},

		{
			...
			func() sdk.Tx {
				...
				gasAmount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712GrantAllowance(from, privKey, "xtechain_9527-1", gas, gasAmount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgEditValidator(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgSubmitEvidence(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712SubmitProposalV1(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgExec(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MsgVoteV1(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MultipleMsgSend(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, true,
		},
		{
			...
			func() sdk.Tx {
				...
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
				txBuilder := suite.CreateTestEIP712MultipleSignerMsgs(from, privKey, "xtechain_9527-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, false,
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "xtechain_9002-1", gas, amount)
				return txBuilder.GetTx()
			}, false, false, false,
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "xtechain_9528-1", gas, amount)
