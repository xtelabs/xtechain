...
func (egcd EthGasConsumeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	if ctx.IsReCheckTx() {
		...
		newCtx := ctx.WithGasMeter(xte.NewInfiniteGasMeterWithLimit(gasWanted))
	...
	blockGasLimit := xte.BlockGasLimit(ctx)
	...
	newCtx := ctx.
		WithGasMeter(xte.NewInfiniteGasMeterWithLimit(gasWanted)).
