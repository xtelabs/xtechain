...
var xteCodec codec.ProtoCodecMarshaler

func init() {
	registry := codectypes.NewInterfaceRegistry()
	xte.RegisterInterfaces(registry)
	xteCodec = codec.NewProtoCodec(registry)
}
...
func VerifySignature(
	...
	case *signing.SingleSignatureData:
		...
		signerChainID, err := xte.ParseChainID(signerData.ChainID)
		if err != nil {
		...
		extOpt, ok := opts[0].GetCachedValue().(*xte.ExtensionOptionsWeb3Tx)
		if !ok {
		...
		typedData, err := eip712.LegacyWrapTxToTypedData(xteCodec, extOpt.TypedDataChainID, msgs[0], txBytes, feeDelegation)
		if err != nil {
