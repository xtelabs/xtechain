...
DEFAULT_GAS_PRICE = "5000000000000axte"
DEFAULT_GAS = "250000"
...
class CosmosCLI:
    ...
    def node_id(self):
        "get xtechain node id"
        output = self.raw("xtechain", "show-node-id", home=self.data_dir)
    ...
    def consensus_address(self):
        "get xtechain consensus address"
        output = self.raw("xtechain", "show-address", home=self.data_dir)
        return output.decode().strip()
    ...
    def balance(self, addr, denom="axte"):
    ...
    def sign_tx_json(self, tx, signer, max_priority_price=None):
        if max_priority_price is not None:
            tx["body"]["extension_options"].append(
                {
                    "@type": "/xte.types.v1.ExtensionOptionDynamicFeeTx",
