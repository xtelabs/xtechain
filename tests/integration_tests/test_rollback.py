...
from .network import setup_custom_xtechain
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:xtechain_9527-1-node{i}":
            ini[section].update(
                {
...
def custom_xtechain(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-xted.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/xted"
    )
    print(broken_binary)
...
def test_rollback(custom_xtechain):
    """
    ...
    print("stop node1")
    supervisorctl(
        custom_xtechain.base_dir / "../tasks.ini", "stop", "xtechain_9527-1-node1"
    )
    ...
    update_node_cmd(custom_xtechain.base_dir, "xted", 1)
    supervisorctl(custom_xtechain.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
