...
def custom_xtechain(tmp_path_factory):
    ...
    yield from setup_xtechain(path, 26700, long_timeout_commit=True)


@pytest.fixture(scope="module", params=["xtechain", "xtechain-ws", "geth"])
def cluster(request, custom_xtechain, geth):
    """
    run on xtechain, xtechain websocket and geth
    """
