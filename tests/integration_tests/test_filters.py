...
from .network import setup_custom_xtechain, setup_xtechain
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_xtechain(tmp_path_factory):
    ...
    yield from setup_xtechain(path, 26200, long_timeout_commit=True)


@pytest.fixture(scope="module")
def xtechain_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module", params=["xtechain", "geth", "xtechain-ws", "enable-indexer"]
)
def cluster(request, custom_xtechain, xtechain_indexer, geth):
    """
    ...
    if provider == "xtechain":
        ...
        xtechain_ws = custom_xtechain.copy()
        xtechain_ws.use_websocket()
        yield xtechain_ws
    elif provider == "enable-indexer":
        yield xtechain_indexer
    else:
