...
from .network import Xtechain, setup_custom_xtechain
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
            ...
            ini[section].update(
                {
                    ...
                    "environment": (
                        f"DAEMON_NAME=xted,DAEMON_HOME=%(here)s/node{i}"
                    ),
                }
            )
...
def custom_xtechain(tmp_path_factory):
    ...
    yield from setup_custom_xtechain(
        path,
        26100,
        Path(__file__).parent / "configs/cosmovisor.jsonnet",
        post_init=post_init,
        chain_binary=str(path / "upgrades/genesis/bin/xted"),
    )


def test_cosmovisor_upgrade(custom_xtechain: Xtechain):
    """
    ...
    """
    cli = custom_xtechain.cosmos_cli()

    w3 = custom_xtechain.w3
    ...
    assert rsp["code"] == 0, rsp["raw_log"]
    # rsp = custom_xtechain.cosmos_cli(1).gov_vote("validator", proposal_id, "yes")
    # assert rsp["code"] == 0, rsp["raw_log"]
    ...
    custom_xtechain.chain_binary = (
        Path(custom_xtechain.chain_binary).parent.parent.parent
        / f"{plan_name}/bin/xted"
    )
    cli = custom_xtechain.cosmos_cli()

    # block should pass the target height
    wait_for_block(cli, target_height + 1, timeout=480)
    wait_for_port(ports.rpc_port(custom_xtechain.base_port(0)))

    # test migrate keystore
    cli.migrate_keystore()

    # check basic tx works after upgrade
    wait_for_port(ports.evmrpc_port(custom_xtechain.base_port(0)))
