...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(
        f"http://localhost:{port}/xte/evm/v1/eth_call", params
    ).json()


def test_grpc_mode(custom_xtechain):
    """
    ...
    api_port = ports.api_port(custom_xtechain.base_port(1))
    ...
    with (custom_xtechain.base_dir / "node1.log").open("a") as logfile:
        proc = subprocess.Popen(
            [
                ...
                "--grpc-only",
                "--home",
                custom_xtechain.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(custom_xtechain.base_port(1))
            ...
            addr = custom_xtechain.cosmos_cli(0).consensus_address()
