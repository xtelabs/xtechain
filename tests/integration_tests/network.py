...
def setup_xtechain(path, base_port, long_timeout_commit=False):
    cfg = Path(__file__).parent / (
        "configs/default.jsonnet"
        if long_timeout_commit
        else "configs/long_timeout_commit.jsonnet"
    )
    yield from setup_custom_xtechain(path, base_port, cfg)
...
def setup_custom_xtechain(
    ...
    try:
        ...
        yield Xtechain(
            path / "xtechain_9527-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
