...
**Prerequisite**: in the repo's root, run `make install` to install the `xted` and `xted` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `xtechain` network. The `test-helper.js` will help you spawn an `xted` process before running the tests.

You can simply run `yarn test --network xtechain` to run all tests with xtechain network, or you can run `yarn test --network ganache` to use ganache shipped with truffle. In most cases, there two networks should produce identical test results.

If you only want to run a few test cases, append the name of tests following by the command line. For example, use `yarn test --network xtechain basic` to run the `basic` test under `xtechain` network.

If you need to take more control, you can also run `xted` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-xtechain`. You should see `xted` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `xted query tx <cosmos-sdk tx>`
...
The [`init-test-node.sh`](./init-test-node.sh) script sets up xtechain with the following accounts:

- `xte10jmp6sgh4cc6zt3e8gw05wavvejgr5pwtu750w` (Validator)
    - `0x7cB61D4117AE31a12E393a1Cfa3BaC666481D02E`
- `xte1cml96vmptgw99syqrrz8az79xer2pcgp767p9e` (User 1)
    - `0xC6Fe5D33615a1C52c08018c47E8Bc53646A0E101`
- `xte1jcltmuhplrdcwp7stlr4hlhlhgd4htqhgjpff2` (user 2)
    - `0x963EBDf2e1f8DB8707D05FC75bfeFFBa1B5BaC17`

Each with roughly 100 ETH available (1e18 xte).

Running `xted keys list --keyring-backend=test` should output:
