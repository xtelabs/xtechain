...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network xtechain', 'run all tests using xtechain network')
    .example('$0 --network xtechain --allowTests=test1,test2', 'run only test1 and test2 using xtechain network')
    .help('h').alias('h', 'help')
    .describe('network', 'set which network to use: ganache|xtechain')
    ...
    .boolean('verbose-log').describe('verbose-log', 'print xted output, default false')
    .argv;
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'xtechain' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or xtechain');
    }
    else {
...
function loadTests(runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach(dirname => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-xtechain'];
...
function performTestSuite({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-xtechain';
...
function setupNetwork({ runConfig, timeout }) {
  if (runConfig.network !== 'xtechain') {
    // no need to start ganache. Truffle will start it
    return;
  }

  // Spawn the xtechain process

  const spawnPromise = new Promise((resolve, reject) => {
    const xtedProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting xted process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      xtedProc.stdout.pipe(process.stdout);
    }
    xtedProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('xted started');
        resolve(xtedProc);
      }
    });
  });
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start xted timeout!')), timeout);
  });
