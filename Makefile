...
SIMAPP = ./app
HTTPS_GIT := https://github.com/xtelabs/xtechain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=xtechain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=$(XTE_BINARY) \
...
docker-build:
	...
	docker rm xtechain || true
	# create a new container from the latest image
	docker create --name xtechain -t -i docker.io/xtechain:latest xtechain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp xtechain:/usr/bin/xted ./build/
...
PACKAGE_NAME:=github.com/xtelabs/xtechain
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/xtelabs/xtechain/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/xtelabs/xtechain/tests/importer
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(XTE_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\xtechain\Z xted/node "./xted testnet --v 4 -o /xtechain --keyring-backend=test --ip-addresses xtednode0,xtednode1,xtednode2,xtednode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(XTE_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/xtechain:Z xted/node "./xted testnet --v 4 -o /xtechain --keyring-backend=test --ip-addresses xtednode0,xtednode1,xtednode2,xtednode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\xted:xtechain\Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\xted:xtechain\Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\xted:xtechain\Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\xted:xtechain\Z xted/node "./xted unsafe-reset-all --home=/xtechain"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/xted:/xtechain:Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/xted:/xtechain:Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/xted:/xtechain:Z xted/node "./xted unsafe-reset-all --home=/xtechain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/xted:/xtechain:Z xted/node "./xted unsafe-reset-all --home=/xtechain"
endif
...
.PHONY: build-docker-local-xtechain localnet-start localnet-stop
